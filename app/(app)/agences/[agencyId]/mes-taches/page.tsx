...
export default function MesTachesPage() {
  ...
  const isLate = useCallback(
    (t: { status: TaskStatus; dueDate: string | null }) =>
      t.status !== "terminee" && !!t.dueDate && t.dueDate < today,
    [today],
  );

  // ✅ Toutes les tâches des projets de CETTE agence (inclut les non assignées)
  const agencyTasks = useMemo(() => {
    const agencyProjectIds = new Set(
      projects.filter((p) => p.agencyId === agencyId).map((p) => p.id)
    );
    return tasks.filter((t) => agencyProjectIds.has(t.projectId));
  }, [tasks, projects, agencyId]);

  // ✅ Mes tâches = tâches assignées à l'utilisateur connecté
  const myTasks = useMemo(() => {
    if (!user) return [];
    const email = user.email.toLowerCase();
    return agencyTasks.filter(
      (t) => !!t.assignedTo && t.assignedTo.toLowerCase() === email
    );
  }, [agencyTasks, user]);

  // ✅ Option 2 : l'admin (administrateur) peut basculer sur l'ensemble des tâches
  const visibleTasks = scope === "all" && isAdmin ? agencyTasks : myTasks;
  ...
  const projectOf = (projectId: string) => projects.find((p) => p.id === projectId);

  const openTaskDetail = (task: { id: string; projectId: string }) =>
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      {/* Vue par statut : filtres colorés par statut (sans bleu dominant) */}
                ...
                {sortedTasks.map((task) => {
                  ...
                  return (
                    <tr
                      ...
                      style={{
                        background: hovered === task.id ? "rgba(100,116,139,0.09)" : undefined,
                        ...
                        {scope === "all" ? (
                            ...
                            <span className="inline-flex items-center gap-2">
                              <span className="w-7 h-7 rounded-full p-[2px] shrink-0" style={{ background: "var(--gradient-primary)" }}>
                                <span
                                  ...
                                  style={assignee.avatar ? { backgroundImage: `url(${assignee.avatar})` } : {}}
                                >
                                  {!assignee.avatar && `${assignee.firstName.charAt(0)}${assignee.lastName.charAt(0)}`}
                                </span>
                              </span>
                          ...
                          <span className="inline-flex items-center gap-2">
                            <span className="w-7 h-7 rounded-full p-[2px] shrink-0" style={{ background: "var(--gradient-primary)" }}>
                              <span
                                className="w-full h-full rounded-full flex items-center justify-center text-white text-[9px] font-bold bg-cover bg-center"
                                style={me?.avatar ? { backgroundImage: `url(${me.avatar})` } : {}}
                              >
                                {!me?.avatar && `${me?.firstName?.charAt(0) ?? "M"}${me?.lastName?.charAt(0) ?? ""}`}
                              </span>
                            </span>
                            <span className="text-xs font-semibold" style={{ color: "var(--text-secondary)" }}>
                              Moi
